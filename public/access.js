(() => {
  ...
  async function loadUsers(){
    ...
    $('#userRows').innerHTML=users.map(u=>`<div class="userRow"><div><b>${esc(u.display_name)}</b><br><small>${esc(u.username)}</small></div><div class="userActions"><span class="roleBadge">${roleLabel(u.global_role)}</span>${u.id!==authUser.id?`<button class="deleteUser danger" data-id="${u.id}" data-name="${esc(u.display_name)}">מחק</button>`:''}</div></div>`).join('')||'<div class="muted">אין משתמשים פעילים</div>';
