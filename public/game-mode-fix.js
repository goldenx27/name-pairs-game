...
(() => {
  ...
  const enhanceWhoRound = () => {
    ...
    lastRound.options.forEach(option => {
      const slot = option.id === lastRound.target.id ? Number(lastRound.imageSlot || 1) : (Math.random() < 0.5 ? 1 : 2);
      ...
      button.onclick = () => {
        ...
        originalButtons.find(b => b.dataset.id === option.id)?.click();
