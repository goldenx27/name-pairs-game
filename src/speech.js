...
const marker=(form,name,fallback=null)=>form.has(name)&&String(form.get(name)).trim()!==''?Number(form.get(name)):fallback;
...
function normalizeHebrew(text){return String(text||'').normalize('NFKD').replace(/[\u0591-\u05C7]/g,'').replace(/[^\u05D0-\u05EA]/g,'');}
...
async function transcribeAttempt(env,audio,target){
  ...
  try{
    ...
    const result=await env.AI.run('@cf/openai/whisper-large-v3-turbo',{audio:bytes,task:'transcribe',language:'he',vad_filter:false,initial_prompt:`הקלטה קצרה מאוד של ילד שחוזר על צליל של אות עברית אחת. האות הצפויה היא ${target}. ייתכן שהצליל יישמע כמו ${target}ה, ${target}י או ${target}ו. תמלל רק את הצליל שנשמע.`});
    const transcript=String(result?.text||'').trim();
    return {...evaluateTranscript(target,transcript),transcript,evaluator:'workers_ai_whisper_large_v3_turbo'};
  }catch(e){console.error('speech evaluation failed',e);return {status:'pending',transcript:'',score:null,evaluator:'workers_ai_error',reason:'workers_ai_error'};}
}

async function serveSpeechMedia(env,url){
  const m=url.pathname.match(/^\/api\/speech-media\/([^/]+)\/(image|prompt)$/);if(!m)return null;
  ...
  const key=m[2]==='image'?row.image_key:row.prompt_audio_key,obj=await env.MEDIA.get(key);if(!obj)return new Response('Not found',{status:404});
  const headers=new Headers();obj.writeHttpMetadata(headers);headers.set('etag',obj.httpEtag);headers.set('cache-control','private, no-cache');return new Response(obj.body,{headers});
}
...
export async function handleSpeechApi(request,env,url){
  ...
  if(request.method==='GET'&&url.pathname==='/api/speech-items'){
    const rows=await env.DB.prepare(`SELECT id,title,target_text,prompt_text,name_start,name_end,response_start,response_end,enabled,created_at,updated_at FROM speech_items WHERE enabled=1 ORDER BY created_at DESC`).all();
  ...
  if(request.method==='POST'&&url.pathname==='/api/speech-items'){
    ...
    const nameStart=marker(form,'nameStart'),nameEnd=marker(form,'nameEnd'),responseStart=marker(form,'responseStart'),responseEnd=marker(form,'responseEnd');
    ...
    await Promise.all([env.MEDIA.put(imageKey,image.stream(),{httpMetadata:{contentType:image.type||'image/jpeg'}}),env.MEDIA.put(audioKey,promptAudio.stream(),{httpMetadata:{contentType:promptAudio.type||'audio/webm'}})]);
    await env.DB.prepare(`INSERT INTO speech_items(id,title,target_text,prompt_text,image_key,prompt_audio_key,name_start,name_end,response_start,response_end) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(itemId,title,targetText,promptText,imageKey,audioKey,nameStart,nameEnd,responseStart,responseEnd).run();
    return json({id:itemId,title,targetText,promptText},201);
  }
  ...
  if(itemMatch&&request.method==='PATCH'){
    ...
    const nameStart=marker(form,'nameStart',row.name_start),nameEnd=marker(form,'nameEnd',row.name_end),responseStart=marker(form,'responseStart',row.response_start),responseEnd=marker(form,'responseEnd',row.response_end);
    const image=form.get('image'),promptAudio=form.get('promptAudio');
    ...
    await env.DB.prepare(`UPDATE speech_items SET title=?,target_text=?,prompt_text=?,name_start=?,name_end=?,response_start=?,response_end=?,updated_at=? WHERE id=?`).bind(title,targetText,promptText,nameStart,nameEnd,responseStart,responseEnd,now(),row.id).run();
    return json({ok:true,id:row.id});
  }
  if(itemMatch&&request.method==='DELETE'){
    const auth=await requireRole(request,env,['ADMIN']);if(auth.error)return auth.error;
    const row=await env.DB.prepare(`SELECT * FROM speech_items WHERE id=?`).bind(itemMatch[1]).first();if(!row)return json({error:'speech_item_not_found'},404);
    await Promise.all([env.MEDIA.delete(row.image_key),env.MEDIA.delete(row.prompt_audio_key)]);await env.DB.prepare(`UPDATE speech_items SET enabled=0,updated_at=? WHERE id=?`).bind(now(),row.id).run();return json({ok:true});
  }
  const attemptMatch=url.pathname.match(/^\/api\/speech-items\/([^/]+)\/attempts$/);
  if(attemptMatch&&request.method==='POST'){
    const form=await request.formData();const playerId=String(form.get('playerId')||''),attemptNo=Math.max(1,Number(form.get('attemptNo')||1)),audio=form.get('audio');
    if(!playerId||!(audio instanceof File)||!audio.size)return json({error:'player_and_audio_required'},400);
    const [player,item]=await Promise.all([env.DB.prepare(`SELECT id FROM players WHERE id=?`).bind(playerId).first(),env.DB.prepare(`SELECT id,target_text FROM speech_items WHERE id=? AND enabled=1`).bind(attemptMatch[1]).first()]);
    if(!player)return json({error:'player_not_found'},404);if(!item)return json({error:'speech_item_not_found'},404);
    const attemptId=id('speech_attempt'),key=`speech_attempts/${playerId}/${item.id}/${attemptId}`,evaluation=await transcribeAttempt(env,audio,item.target_text);
    await env.MEDIA.put(key,audio.stream(),{httpMetadata:{contentType:audio.type||'audio/webm'}});
    await env.DB.prepare(`INSERT INTO speech_attempts(id,player_id,item_id,attempt_no,response_audio_key,transcript,score,result,evaluator,evaluated_at) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(attemptId,playerId,item.id,attemptNo,key,evaluation.transcript||null,evaluation.score,evaluation.status,evaluation.evaluator,evaluation.status==='pending'?null:now()).run();
