...
async function sha256(value) { return bytesToHex(new Uint8Array(await crypto.subtle.digest('SHA-256', enc.encode(String(value))))); }
...
export async function verifyPassword(password, salt, expectedHash) {
  ...
  const {hash} = await hashPassword(password, salt); if (hash.length !== expectedHash.length) return false;
  let diff=0; for(let i=0;i<hash.length;i++) diff|=hash.charCodeAt(i)^expectedHash.charCodeAt(i); return diff===0;
}
function cookieToken(request){const raw=request.headers.get('cookie')||'';for(const part of raw.split(';')){const [k,...rest]=part.trim().split('=');if(k==='crew_session')return decodeURIComponent(rest.join('='));}return '';}
function setCookie(token,maxAge=60*60*24*30){return `crew_session=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${maxAge}`;}
function clearCookie(){return 'crew_session=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0';}
export async function createSession(env,userId,userAgent=''){const token=`${crypto.randomUUID()}${crypto.randomUUID().replaceAll('-','')}`,tokenHash=await sha256(token),sessionId=id('auth'),expiresAt=new Date(Date.now()+30*24*60*60*1000).toISOString();await env.DB.prepare(`INSERT INTO auth_sessions(id,user_id,token_hash,expires_at,last_seen_at,user_agent) VALUES(?,?,?,?,?,?)`).bind(sessionId,userId,tokenHash,expiresAt,now(),String(userAgent).slice(0,500)).run();return {token,expiresAt};}
export async function currentUser(request,env){const token=cookieToken(request);if(!token)return null;const tokenHash=await sha256(token);const row=await env.DB.prepare(`SELECT u.id,u.username,u.display_name,u.global_role,u.active,s.id AS session_id,s.expires_at FROM auth_sessions s JOIN app_users u ON u.id=s.user_id WHERE s.token_hash=? AND u.active=1 AND s.expires_at > CURRENT_TIMESTAMP`).bind(tokenHash).first();if(!row)return null;await env.DB.prepare(`UPDATE auth_sessions SET last_seen_at=? WHERE id=?`).bind(now(),row.session_id).run();return {id:row.id,username:row.username,displayName:row.display_name,role:row.global_role,sessionId:row.session_id};}
export async function requireRole(request,env,roles){const user=await currentUser(request,env);if(!user)return {error:json({error:'unauthorized'},401)};if(roles?.length&&!roles.includes(user.role))return {error:json({error:'forbidden'},403)};return {user};}
async function verifyLegacyPin(env,familyId,pin){if(!familyId||!pin)return false;const row=await env.DB.prepare(`SELECT parent_pin_hash FROM families WHERE id=?`).bind(familyId).first();return !!row?.parent_pin_hash&&row.parent_pin_hash===await sha256(pin);}
async function createUser(env,{username,displayName,role,password,createdBy}){username=String(username||'').trim().toLowerCase();displayName=String(displayName||username).trim();if(!/^[a-z0-9._-]{3,40}$/i.test(username))throw new Error('invalid_username');if(!['ADMIN','PARENT','CHILD'].includes(role))throw new Error('invalid_role');if(role!=='CHILD'&&String(password||'').length<6)throw new Error('password_too_short');const userId=id('user');let passwordHash=null,passwordSalt=null;if(role!=='CHILD'){const hp=await hashPassword(password);passwordHash=hp.hash;passwordSalt=hp.salt;}await env.DB.prepare(`INSERT INTO app_users(id,username,display_name,global_role,password_hash,password_salt,created_by) VALUES(?,?,?,?,?,?,?)`).bind(userId,username,displayName,role,passwordHash,passwordSalt,createdBy||null).run();return {id:userId,username,displayName,role};}
async function canSeePlayer(env,user,playerId){if(user.role==='ADMIN')return true;if(user.role!=='PARENT')return false;return !!(await env.DB.prepare(`SELECT 1 FROM parent_children WHERE parent_user_id=? AND child_player_id=? LIMIT 1`).bind(user.id,playerId).first());}

export async function handleAuthApi(request,env,url){
 const path=url.pathname;
 if(request.method==='GET'&&path==='/api/auth/status'){const admin=await env.DB.prepare(`SELECT id FROM app_users WHERE global_role='ADMIN' AND active=1 LIMIT 1`).first();return json({needsAdminSetup:!admin,user:await currentUser(request,env)});}
 if(request.method==='POST'&&path==='/api/auth/setup-admin'){const existing=await env.DB.prepare(`SELECT id FROM app_users WHERE global_role='ADMIN' AND active=1 LIMIT 1`).first();if(existing)return json({error:'admin_already_exists'},409);const b=await request.json().catch(()=>({}));if(!(await verifyLegacyPin(env,b.familyId,b.pin)))return json({error:'wrong_pin'},403);try{const user=await createUser(env,{username:b.username,displayName:b.displayName||b.username,role:'ADMIN',password:b.password});const s=await createSession(env,user.id,request.headers.get('user-agent')||'');return json({user},201,{'set-cookie':setCookie(s.token)});}catch(e){return json({error:String(e.message||e)},400);}}
 ...
 if(request.method==='POST'&&path==='/api/manage/activate-child'){const auth=await requireRole(request,env,['ADMIN','PARENT']);if(auth.error)return auth.error;const b=await request.json().catch(()=>({}));if(!(await canSeePlayer(env,auth.user,b.playerId)))return json({error:'forbidden'},403);const p=await env.DB.prepare(`SELECT id,family_id,name FROM players WHERE id=?`).bind(b.playerId).first();return p?json({player:p}):json({error:'player_not_found'},404);}
 return null;
}
